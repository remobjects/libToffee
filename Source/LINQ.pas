...
extension method Foundation.INSFastEnumeration.Prepend(aItem: id): not nullable Foundation.INSFastEnumeration; iterator; public;
...
extension method RemObjects.Elements.System.INSFastEnumeration<T>.Prepend(aItem: T): not nullable RemObjects.Elements.System.INSFastEnumeration<T>; inline; public;
...
extension method Foundation.INSFastEnumeration.SkipLast(aCount: NSInteger): not nullable Foundation.INSFastEnumeration;
begin
  var lArray := self.ToNSArray();
  var lCount := lArray.count-aCount;
  if lCount < 0 then
    lCount := 0;
...
extension method Foundation.INSFastEnumeration.SelectMany(aBlock: not nullable IDBlock): not nullable Foundation.INSFastEnumeration;
begin
  for each i in self do begin
...
extension method Foundation.INSFastEnumeration.Prepend(aItem: id): not nullable Foundation.INSFastEnumeration;
begin
  yield aItem;
  for each i in self do
    yield i;
...
begin
  ...
  if lFirst.count <> lSecond.count then
    exit false;

  for i: NSInteger := 0 to lFirst.count-1 do begin
    var lA := lFirst[i];
    var lB := lSecond[i];
...
extension method Foundation.INSFastEnumeration.ElementAt(aIndex: NSInteger): nullable id;
begin
  if aIndex < 0 then
    raise new Exception("Index was out of range.");

  if self is NSArray then begin
    var lArray := self as NSArray;
    if aIndex >= lArray.count then
      raise new Exception("Index was out of range.");
    exit lArray[aIndex];
  end;

  var lIndex: NSInteger := 0;
  for each i in self do begin
...
extension method Foundation.INSFastEnumeration.ElementAtOrDefault(aIndex: NSInteger): nullable id;
begin
  if aIndex < 0 then
    exit nil;
...
extension method Foundation.INSFastEnumeration.GroupJoin(aSecond: not nullable Foundation.INSFastEnumeration; aSelfBlock: not nullable IDBlock; aSecondBlock: not nullable IDBlock; aResultBlock: not nullable ID2Block): not nullable Foundation.INSFastEnumeration;
begin
  var lLookup := new NSMutableDictionary;
  var lEmptyGroup := new NSMutableArray;
  ...
  for each i in self do begin
    ...
    var lGroup := lLookup[lKey] as NSArray;
    yield aResultBlock(i, if assigned(lGroup) then lGroup else lEmptyGroup);
  end;
end;
...
extension method RemObjects.Elements.System.INSFastEnumeration<T>.Prepend(aItem: T): not nullable RemObjects.Elements.System.INSFastEnumeration<T>;
begin
  exit Foundation.INSFastEnumeration(self).Prepend(aItem);
end;
...
begin
  exit Foundation.INSFastEnumeration(self).SequenceEqual(Foundation.INSFastEnumeration(aSecond));
end;
...
extension method RemObjects.Elements.System.INSFastEnumeration<T>.ElementAt(aIndex: NSInteger): {nullable} T;
begin
  exit Foundation.INSFastEnumeration(self).ElementAt(aIndex);
end;
...
extension method RemObjects.Elements.System.INSFastEnumeration<T>.GroupJoin<T, TInner, TKey, TResult>(aSecond: not nullable RemObjects.Elements.System.INSFastEnumeration<TInner>; aSelfBlock: not nullable block(aItem: not nullable T): TKey; aSecondBlock: not nullable block(aItem: not nullable TInner): TKey; aResultBlock: not nullable block(aItem: not nullable T; aGroup: not nullable RemObjects.Elements.System.INSFastEnumeration<TInner>): TResult): not nullable RemObjects.Elements.System.INSFastEnumeration<TResult>;
begin
  exit Foundation.INSFastEnumeration(self).GroupJoin(Foundation.INSFastEnumeration(aSecond), IDBlock(aSelfBlock), IDBlock(aSecondBlock), ID2Block((a, b) -> aResultBlock(T(a), RemObjects.Elements.System.INSFastEnumeration<TInner>(b))));
end;
